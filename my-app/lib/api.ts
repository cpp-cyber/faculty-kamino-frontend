...
export async function deployPod(templateName: string): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to deploy pod: ${response.status} ${response.statusText}`,
      );
    }
  } finally {
...
export async function deletePod(podNames: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete pod: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function getDashboardData(): Promise<DashboardResponse> {
  return await deduplicatedFetch("dashboardData", () =>
...
export async function getProxmoxResources(): Promise<ProxmoxResourcesResponse> {
  return await deduplicatedFetch("proxmoxResources", () =>
    fetch("/api/v1/admin/cluster", {
      cache: "no-store",
      credentials: "include",
    }),
  );
...
export async function getAllUsers(): Promise<GetUsersResponse> {
  const data: GetUsersResponse = await deduplicatedFetch("allUsers", () =>
...
export async function deleteUsers(usernames: string[]): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete users: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function updateUserGroups(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to update user groups: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function getGroups(): Promise<GetGroupsResponse> {
  const data: GetGroupsResponse = await deduplicatedFetch("allGroups", () =>
...
export async function createGroups(groupNames: string[]): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create groups: ${response.status} ${response.statusText}`,
    );
  }
}

// Add users to groups (bulk operation)
export async function addUsersToGroups(
  usernames: string[],
  groupName: string,
): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to add users to group: ${response.status} ${response.statusText}`,
    );
  }
}

// Remove users from groups (bulk operation)
export async function removeUsersFromGroups(
  usernames: string[],
  groupName: string,
): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to remove users from group: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function renameGroup(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to rename group: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function editGroup(
  ...
  const response = await fetch(`/api/v1/admin/group/edit`, {
    method: "POST",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ group: groupName, comment: comment }),
  });

  if (!response.ok) {
    throw new Error(
      `Failed to edit group: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function deleteGroups(groupNames: string[]): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete groups: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function startVM(vmid: number, node: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to start VM: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function shutdownVM(vmid: number, node: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to shutdown VM: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function rebootVM(vmid: number, node: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to reboot VM: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function adminDeletePods(podNames: string[]): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete pods: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function getUnpublishedTemplates(): Promise<
  UnpublishedPodTemplate[]
> {
...
export async function publishTemplate(template: PodTemplate): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to publish templates: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function editTemplate(template: PodTemplate): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to edit template: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function toggleTemplateVisibility(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to toggle template visibility: ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function clonePodTemplates(
  template: string,
  usernames: string[],
  groups: string[],
  startingVmId?: number,
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to clone pod templates: ${response.status} ${response.statusText}`,
      );
    }
  } finally {
    clearTimeout(timeoutId);
...
// =============================================================================
// ADMIN PROXMOX ENDPOINTS
// =============================================================================

// Get available Proxmox VM templates
export async function getProxmoxTemplates(): Promise<{
  ...
  return { templates: data.vm_templates || [] };
}

// Get existing Proxmox template pool names
export async function getProxmoxTemplateNames(): Promise<string[]> {
  ...
  return data.template_pools || [];
}

// Get existing VNets
export async function getVNets(): Promise<
  Array<{ vnet: string; tag: number }>
> {
  const data: { status: string; vnets: Array<{ vnet: string; tag: number }> } =
    await deduplicatedFetch("vnets", () =>
      fetch("/api/v1/admin/vnets", {
        cache: "no-store",
        credentials: "include",
      }),
    );
...
// Create new Kamino template from Proxmox VMs
export async function createKaminoTemplateInProxmox(templateData: {
  name: string;
  add_router: boolean;
